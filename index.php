<?php
    session_start();
    $_SESSION['load'] = true;

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="author" content="Gianni Ramdjiawan">
    <!-- Oswald font -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Oswald:wght@300&display=swap" rel="stylesheet">
    <!-- Shippori Antique B1 font -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Serif+JP&display=swap" rel="stylesheet">
    <!-- bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet"
        integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"
        integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p"
        crossorigin="anonymous"></script>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.8.3/font/bootstrap-icons.css">
    <link rel="stylesheet" href="./css/style.css">
    <script src="./js/main.js" defer></script>
    <link rel="shortcut icon" href="./img/logo-portfolio-png.svg" type="image/x-icon">
    <title>Protfolio: Gianni Ramdjiawan</title>
    <meta name="description"
        content="Welcome to my portfolio. My name is Gianni Ramdjiawan and this started as a school project but now it has become my opus magnum.">
</head>
<body>
    <div class="container-fluid px-0 overflow-hidden home-page pofo" id="home-page">
        <div class="row contact">
            <div class="col-md-6 p-1 px-2">
                <span class="mx-2">
                    <i class="bi bi-envelope-fill" style="color: white;"></i>
                    <a class="contact-link" href="mailto:gianni.ramdjiawan@gmail.com" target="_blank">Contact Me</a>
                </span>
                <span class="mx-2">
                    <i class="bi bi-linkedin" style="color: white;"></i>
                    <a class="contact-link" href="https://www.linkedin.com/in/gianni-ramdjiawan-712247199/"
                        target="_blank">Find Me</a>
                </span>
                <span class="mx-2">
                    <i class="bi bi-github" style="color: white;"></i>
                    <a class="contact-link" href="https://github.com/GRamdjiawan" target="_blank">Find Me</a>
                </span>
            </div>
            <div class="col-md-6"></div>
        </div>
        <nav class="navbar navbar-expand-lg navbar-dark mx-0 p-4 border-bottom border-dark">
            <div class="container-fluid">
                <a class="navbar-brand" href="./index.php">
                    <img src="./img/logo-portfolio-png.svg" alt="Gianni Ramdjiawan" class="noselect" width="200"
                        height="200">
                </a>
                <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNav"
                    aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon text-dark"></span>
                </button>
                <div class="collapse navbar-collapse" id="navbarNav">
                    <ul class="navbar-nav">
                        <li class="nav-item">
                            <a class="nav-link" href="#">Home</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="index.php#project-page">Projects</a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
        <img src="./img/wavetop.svg" alt="" class="img-fluid noselect">
        <div class="row about-me">
            <div class="col-md-6 m-0  d-flex justify-content-center align-items-center">
                <div class="about-me-content p-3 mx-4  d-flex flex-column justify-content-center align-items-center">
                    <h2 class="display-4">About me</h2>
                    <p class="text-break fs-4 p-5 about-me-text">
                        My name is Gianni Ramdjiawan,
                        I'm a <span class="age"></span>-year old student
                        who loves to learn new things. My motto is: as quick and
                        efficient possible. When I start something I'll do
                        anything to finish it.
                        <br><br>
                        This is my <a href="./html/cv.html" class="text-dark"><b>cv</b></a>.
                    </p>
                </div>
            </div>
            <div class="col-md-6 p-5 m-0 d-flex justify-content-center align-items-center">
                <a href="#" data-bs-toggle="tooltip" data-bs-placement="bottom" title="IT'S A ME! ">
                    <img src="./img/foto-van-zelf.jpeg" alt="Gianni Ramdjiawan" class="img-fluid img-me noselect">
                </a>
            </div>
        </div>
        <div class="row">
            <img src="./img/wave.svg" alt="wave" class="p-0 img-fluid noselect">
        </div>
        <div class="row " id="project-page">
            <div class="col-md-12 text-center">
                <h2 class="display-4 mb-5">Projects</h2>
                <div class="projects-container d-flex justify-content-evenly align-items-center">
                    <button class="btn back">
                        <i class="bi bi-arrow-bar-left fs-1"></i>
                    </button>
                    <div class="projects-content">
                    </div>
                    <button class="btn next">
                        <i class="bi bi-arrow-bar-right fs-1"></i>
                    </button>
                </div>
            </div>
        </div>
        <div class="row">
            <img src="./img/wavetop.svg" alt="wave" class="p-0 noselect img-fluid">
        </div>
    </div>
</body>
</html>